...
def create_event_with_recurrence(mysql, event_data, bulletins_data=None):
    ...
    try:
        ...
        try:
            ...
            # If DESCRIBE fails, continue; table might be created elsewhere
...
def has_conflict_with_recurrence(mysql, event_date, start_time, end_time, recurrence_type, 
    ...
    base_conflicts = check_single_date_conflicts(cur, event_date, start_time, end_time, exclude_event_id, location)
...
def check_single_date_conflicts(cursor, event_date, start_time, end_time, exclude_event_id=None, location=None):
    """
...
# Update the get_events() function to include location field
        ...
        for row in events:
            if isinstance(row, dict):
                event_data = {
                    ...
                    "location": row.get('location')  # Add location to response
                }
                if show_series:
                ...
                if show_series:
                    event_data = {
                        ...
                        "location": row[9] if len(row) > 9 else None  # Add location from tuple
                    }
                else:
                    event_data = {
                        "id": row[0],
...
def current_activity_by_location():
    try:
        ...
        # Determine current/next activity per location
        results = {}
...
def check_single_date_conflicts_with_location(cursor, event_date, start_time, end_time, exclude_event_id=None, location=None):
    ...
    # Only treat as conflict if location matches (same room) or if either location is null
    if location:
